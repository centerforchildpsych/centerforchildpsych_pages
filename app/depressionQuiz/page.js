...
const CONFIG = {
  ...
  items: [
    "had little interest or pleasure in things they used to enjoy?",
    "seemed sad, down, low, or hopeless?",
    "had trouble with sleep, either sleeping too little or too much?",
    "seemed tired or had little energy?",
    "had changes in appetite or weight?",
    "expressed feelings of worthlessness or guilt?",
    "had trouble concentrating on everyday tasks?",
    "been unusually irritable or easily frustrated?",
  ],
...
export default function DepressionQuizPage() {
  ...
  /*
   * This matches the working OCD quiz structure.
   */
  const steps = useMemo(() => {
    const allSteps = [];
   ...
   * ---------------------------------------------------------
   *
   * The safety question is handled separately.
   *
   * Number(answer) > 0 means the safety flag is true.
   *
   * The safety answer is NEVER added to the depression score.
   */
  ...
  /*
   * Keep the same generic trailing-question behavior
   * as the working OCD/Bipolar quiz.
   */
  steps.forEach((currentStep, index) => {
    if (
  ...
  return (
    <>
      ...
      <div className={styles.resultCtaSection}>
        ...
        >
          <a
            className={`${styles.btn} ${styles.btnSolid} ${styles.btnNavy}`}
